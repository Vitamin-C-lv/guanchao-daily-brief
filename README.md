...
一个面向个人使用的响应式市场简报，覆盖美联储政策、A 股、港股、美股和近期财经热点。早间生成每日早报，交易日晚间补充 A/H 收盘、机构观点与板块轮动，每周六上午生成深度周报。页面按参考图采用浅紫灰背景、白色大卡片、黑色粗标题与柔和渐变；桌面端使用侧栏和多栏仪表盘，手机竖屏使用单列卡片、横滑市场卡和底部导航。
...
- 每周六 10:00 由 `gpt-5.6-terra` 读取本周日报归档、A/H 最后完整收盘与美国周五完整收盘并检索截至发布边界的正式信息，发布独立周报；规则见 `content/WEEKLY_AUTOMATION_PROMPT.md`。
