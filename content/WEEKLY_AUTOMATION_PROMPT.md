...
- 分别审阅 A 股、港股的 5/20 交易日窗口：样本覆盖率、缺失率、预测分布、方向分布、到期项数量、排序相关性/Top-Bottom 命中或版本 model card 预先定义的指标。A 股 `current` 可复盘同一最新完整交易日的可用子集，但必须记录“`N/12` 项可比”；5/20 日预测只有在 `a-core12-v2` 全 12 项完整、taxonomy hash 与冻结 artifact/model card 匹配且相应窗口回测门禁已通过时才算已发布预测。样本太少、交易日未到、标签不完整或任一预测门禁失败时写 `insufficient`，不报虚假精度。
...
- 周报中的 5/20 交易日预测排序仍必须分别满足 `a-core12-v2` 完整 12 项输入、taxonomy hash 与冻结 artifact/model card 完全匹配，以及相应 horizon 的预登记样本外回测门禁；`current` 的部分覆盖不得降级进入预测。
