...
const GUARDIAN = "PUBLISHER_GUARDIAN MAX_GUARDIAN_PUBLISHER_RETRY=1 PUBLISHER_STILL_RUNNING publisher-guardian.mjs SUNDAY_NO_RUN";

function toml(id, rrule, prompt) {
  return `id = "${id}"\nstatus = "ACTIVE"\nrrule = "${rrule}"\nmodel = "gpt-5.6-luna"\nprompt = '''${prompt}'''\ncwds = ["D:/Guanchao-Workspace/repo/guanchao-daily-brief", "D:/Guanchao-Workspace/runtime/local-writer-runtime"]\n`;
}

function fixture({ scheduler = null, dailyRrule = "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=20;BYMINUTE=0", nativeDailyRrule = dailyRrule, handoverDailyRrule = nativeDailyRrule, dailyPrompt = DAILY, skill = true } = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "automation-consistency-v2-"));
  const repo = path.join(root, "repo");
  fs.mkdirSync(path.join(repo, "config"), { recursive: true });
  fs.mkdirSync(path.join(repo, "docs"), { recursive: true });
  const config = { schemaVersion: "codex-writer-automation-v2", publicationEnabled: true, productionApplyRequiresExplicitWrite: true, runtime: { projectPath: "D:/Guanchao-Workspace/runtime/local-writer-runtime", repositoryPath: "D:/Guanchao-Workspace/repo/guanchao-daily-brief" }, writer: { model: "gpt-5.6-luna" }, prediction: { model: null, normalPathUsesLlm: false, normalPathLlmTokens: 0 }, schedules: [{ key: "prediction", executor: "windows-task-scheduler", taskName: "Guanchao Prediction Publisher 18-20", enabled: true, rrule: "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=18;BYMINUTE=20", promptFile: "prompts/codex-prediction-publisher.md" }, { key: "guardian", executor: "codex-automation", automationId: "codex-3", enabled: true, rrule: "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=18;BYMINUTE=30", promptFile: "prompts/codex-publisher-guardian.md" }, { key: "daily", executor: "codex-automation", automationId: "codex", enabled: true, rrule: dailyRrule, promptFile: "prompts/codex-daily-writer.md" }, { key: "weekly", executor: "codex-automation", automationId: "codex-2", enabled: true, rrule: "FREQ=WEEKLY;BYDAY=SA;BYHOUR=10;BYMINUTE=0", promptFile: "prompts/codex-weekly-writer.md" }], handover: { status: "production-cutover-active", candidateSchedulesAreNotProductionActive: false, activeProduction: { prediction: { automationId: "Guanchao Prediction Publisher 18-20", rrule: "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=18;BYMINUTE=20" }, guardian: { automationId: "codex-3", rrule: "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=18;BYMINUTE=30" }, daily: { automationId: "codex", rrule: handoverDailyRrule }, weekly: { automationId: "codex-2", rrule: "FREQ=WEEKLY;BYDAY=SA;BYHOUR=10;BYMINUTE=0" } } } };
  fs.writeFileSync(path.join(repo, "config", "codex-writer-automation.json"), `${JSON.stringify(config, null, 2)}\n`);
  fs.writeFileSync(path.join(repo, "config", "report-availability.json"), `${JSON.stringify({ schemaVersion: "guanchao-report-availability-v1", enabled: true, mode: "availability_first", manualDisableOnly: true }, null, 2)}\n`);
  ...
  fs.writeFileSync(path.join(repo, "prompts", "codex-publisher-guardian.md"), GUARDIAN, "utf8");
  const automations = path.join(root, "automations");
  for (const [id, rule, prompt] of [["codex", nativeDailyRrule, dailyPrompt], ["codex-2", "FREQ=WEEKLY;BYDAY=SA;BYHOUR=10;BYMINUTE=0", WEEKLY], ["codex-3", "FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR,SA;BYHOUR=18;BYMINUTE=30", GUARDIAN]]) { fs.mkdirSync(path.join(automations, id), { recursive: true }); fs.writeFileSync(path.join(automations, id, "automation.toml"), toml(id, rule, prompt), "utf8"); }
  const state = { dailyAutomationId: "codex", weeklyAutomationId: "codex-2", guardianAutomationId: "codex-3", configSha256: sha256AutomationConfigBytes(fs.readFileSync(path.join(repo, "config", "codex-writer-automation.json"))), prompts: { daily: { sha256: sha256(dailyPrompt) }, weekly: { sha256: sha256(WEEKLY) }, guardian: { sha256: sha256(GUARDIAN) } }, enabled: true, review: { handoverStatus: "production-cutover-active" }, runtime: { projectPath: "D:/Guanchao-Workspace/runtime/local-writer-runtime", repositoryPath: "D:/Guanchao-Workspace/repo/guanchao-daily-brief" } };
  ...
  return { repo, automations, stateFile, skillDirectory, scheduler: scheduler ?? (() => ({ exists: true, taskName: "Guanchao Prediction Publisher 18-20", status: "Ready", taskToRun: "powershell run-prediction-publisher-task.ps1 -Mode Write", schedule: "Weekly Mon,Tue,Wed,Thu,Fri,Sat 18:20" })), cleanup: () => fs.rmSync(root, { recursive: true, force: true }) };
}
