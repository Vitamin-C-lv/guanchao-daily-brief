...
function fixture({ scheduler = null, dailyRrule = "FREQ=DAILY;BYHOUR=20;BYMINUTE=0", nativeDailyRrule = "FREQ=DAILY;BYHOUR=7;BYMINUTE=30", handoverDailyRrule = nativeDailyRrule, dailyPrompt = DAILY, skill = true } = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "automation-consistency-v2-"));
  const repo = path.join(root, "repo");
  fs.mkdirSync(path.join(repo, "config"), { recursive: true });
  fs.mkdirSync(path.join(repo, "docs"), { recursive: true });
  const config = { schemaVersion: "codex-writer-automation-v2", publicationEnabled: true, productionApplyRequiresExplicitWrite: true, runtime: { projectPath: "D:/Guanchao-Workspace/runtime/local-writer-runtime", repositoryPath: "D:/Guanchao-Workspace/repo/guanchao-daily-brief" }, writer: { model: "gpt-5.6-luna" }, prediction: { model: null, normalPathUsesLlm: false, normalPathLlmTokens: 0 }, schedules: [{ key: "prediction", executor: "windows-task-scheduler", taskName: "Guanchao Prediction Publisher 18-20", enabled: false, rrule: "FREQ=DAILY;BYHOUR=18;BYMINUTE=20", promptFile: "prompts/codex-prediction-publisher.md" }, { key: "daily", executor: "codex-automation", automationId: "codex", enabled: false, rrule: dailyRrule, promptFile: "prompts/codex-daily-writer.md" }, { key: "weekly", executor: "codex-automation", automationId: "codex-2", enabled: false, rrule: "FREQ=WEEKLY;BYDAY=SA;BYHOUR=10;BYMINUTE=0", promptFile: "prompts/codex-weekly-writer.md" }], handover: { status: "pre-merge-safe", activeProduction: { prediction: { automationId: "guanchao-prediction-publisher", rrule: "FREQ=DAILY;BYHOUR=6;BYMINUTE=45" }, daily: { automationId: "codex", rrule: handoverDailyRrule }, weekly: { automationId: "codex-2", rrule: "FREQ=WEEKLY;BYDAY=SA;BYHOUR=10;BYMINUTE=0" } } } };
  ...
  return { repo, automations, stateFile, skillDirectory, scheduler: scheduler ?? (() => ({ exists: true, taskName: "Guanchao Prediction Publisher 18-20", status: "Disabled", taskToRun: "powershell run-prediction-publisher-task.ps1 -Mode DryRun", schedule: "Daily 18:20" })), cleanup: () => fs.rmSync(root, { recursive: true, force: true }) };
}
