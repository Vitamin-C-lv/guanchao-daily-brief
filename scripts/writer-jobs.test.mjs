...
test("derived countdown field change passes without evidence rebinding", () => { const fixture = setup(); try { const value = result(fixture, { unchanged: true }); value.payload.federalReserve.countdownDays += 1; assert.doesNotThrow(() => validateResult(fixture.repo, fixture.request, sealWriterResult(value))); } finally { cleanup(fixture); } });
...
test("64 weekly publication result requires report and index together", () => { const fixture = setup({ edition: "weekly" }); try { const applied = apply({ rootDir: fixture.repo, request: fixture.request, result: result(fixture), write: true }); assert.deepEqual(applied.weeklyPublication, { reportWritten: true, indexUpdated: true, latestReportId: "weekly-2026-W29", archiveUpdated: true, weeklyPublished: true }); } finally { cleanup(fixture); } });
test("65 accepted weekly result repairs an index that lost the report", () => { const fixture = setup({ edition: "weekly" }); try { const first = result(fixture); apply({ rootDir: fixture.repo, request: fixture.request, result: first, write: true }); const indexPath = path.join(fixture.repo, "content/weekly-reports/index.json"); const index = JSON.parse(fs.readFileSync(indexPath, "utf8")); index.latestReportId = null; index.reports = []; fs.writeFileSync(indexPath, `${JSON.stringify(index, null, 2)}\n`); const repaired = apply({ rootDir: fixture.repo, request: fixture.request, result: first, write: true }); assert.equal(repaired.weeklyPublication.weeklyPublished, true); assert.equal(JSON.parse(fs.readFileSync(indexPath, "utf8")).latestReportId, "weekly-2026-W29"); } finally { cleanup(fixture); } });
test("66 weekly publication fails closed when an index entry has no report file", () => { const fixture = setup({ edition: "weekly" }); try { const indexPath = path.join(fixture.repo, "content/weekly-reports/index.json"); const index = JSON.parse(fs.readFileSync(indexPath, "utf8")); index.reports.push({ ...index.reports[0], id: "weekly-2026-W30", weekStart: "2026-07-27", weekEnd: "2026-08-01" }); index.latestReportId = "weekly-2026-W30"; fs.writeFileSync(indexPath, `${JSON.stringify(index, null, 2)}\n`); const noticesPath = path.join(fixture.repo, "public/update-notices.json"); const notices = JSON.parse(fs.readFileSync(noticesPath, "utf8")); notices.weekly.href = "/weekly/weekly-2026-W30/"; fs.writeFileSync(noticesPath, `${JSON.stringify(notices, null, 2)}\n`); assert.equal(codeOf(() => apply({ rootDir: fixture.repo, request: fixture.request, result: result(fixture), write: true })), "WEEKLY_PUBLICATION_INVALID"); } finally { cleanup(fixture); } });
