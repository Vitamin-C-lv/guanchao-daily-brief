...
def build_weekly_review(
    ...
    if not re.fullmatch(r"\d{4}-W\d{2}", iso_week):
        raise LedgerError("isoWeek must be YYYY-Www")
    ...
    scored_calls = []
    for record, event in current_pairs:
        scored_calls.append({
            "predictionId": record["predictionId"], "market": record["market"], "sectorId": record["sectorId"],
            "modelVersion": record["modelVersion"], "horizonSessions": record["horizonSessions"],
            "probabilityTarget": record["probabilityTarget"], "result": event["result"],
            "realizedExcessReturn": event["realizedExcessReturn"],
        })
    ...
    machine_recommendations = {
        ...
        "regimeInstability": [],
        "recommendedResearchActions": ["accumulate_same_target_outcomes", "audit_feature_coverage_without_lowering_gates"],
    ...
    return {
        ...
        "counts": {key: counts[key] for key in required_counts},
        "metrics": metrics,
        "metricReasons": metric_reasons,
        "targetMetrics": target_metrics,
        "slices": slices,
        "modelVersionComparison": versions,
        "abstainReasonDistribution": dict(sorted(abstain_distribution.items())),
        "largestErrors": largest_errors,
        "bestCalls": best_calls,
        "recommendations": recommendations,
        "machineRecommendations": machine_recommendations,
        "policy": {
            "legacyExcludedFromCurrentMetrics": True,
