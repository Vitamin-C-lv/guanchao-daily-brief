...
test("85 effective publication keeps the source calendar date", () => assert.equal(contract.effectivePublicationDate({ publishedDate: "2026-07-30", publishedAt: "2026-07-31T03:30:00.000Z" }), "2026-07-30"));
test("86 timestamp-only publication uses Shanghai date", () => assert.equal(contract.effectivePublicationDate({ publishedDate: null, publishedAt: "2026-07-31T16:30:00.000Z" }), "2026-08-01"));
test("87 null publication sorts after dated documents", () => { const dated = { documentId: "z", publishedDate: "2026-07-30", publishedAt: null }; const missing = { documentId: "a", publishedDate: null, publishedAt: null }; assert.equal([missing, dated].sort(contract.compareDocumentPublication)[0], dated); });
test("88 same-day precise publication sorts before date-only", () => { const precise = { documentId: "z", publishedDate: "2026-07-30", publishedAt: "2026-07-30T12:00:00.000Z" }; const dateOnly = { documentId: "a", publishedDate: "2026-07-30", publishedAt: null }; assert.equal([dateOnly, precise].sort(contract.compareDocumentPublication)[0], precise); });
test("89 precise publications sort by timestamp", () => { const early = { documentId: "z", publishedDate: "2026-07-30", publishedAt: "2026-07-30T01:00:00.000Z" }; const late = { documentId: "a", publishedDate: "2026-07-30", publishedAt: "2026-07-30T02:00:00.000Z" }; assert.equal([late, early].sort(contract.compareDocumentPublication)[0], early); });
test("90 publication ties sort by document ID", () => { const first = { documentId: "a", publishedDate: null, publishedAt: null }; const second = { documentId: "b", publishedDate: null, publishedAt: null }; assert.equal([second, first].sort(contract.compareDocumentPublication)[0], first); });
