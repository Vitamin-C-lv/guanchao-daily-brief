...
export default function LineComparisonChart({ visual }: { visual: ArticleVisual }) {
  const width = 640;
  const height = 300;
  ...
  const line = (seriesId: string) =>
    xValues
      .map((x, index) => {
        const point = visual.points.find((item) => item.x === x && item.seriesId === seriesId);
        if (!point || point.y === null) return null;
        return `${index === 0 ? "M" : "L"}${xAt(x).toFixed(1)},${yAt(point.y).toFixed(1)}`;
      })
      .filter(Boolean)
      .join(" ");
      ...
      {!hasData ? (
        ...
        >
          ...
          {visual.points.map((point) =>
            point.y === null ? null : (
              <circle key={`${point.x}-${point.seriesId}`} cx={xAt(point.x)} cy={yAt(point.y)} r={3.5} className="article-chart-dot" />
            ),
          )}
          {xValues.map((x) => (
