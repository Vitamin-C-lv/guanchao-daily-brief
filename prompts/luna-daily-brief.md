...
Your only inputs are one `writer-request-v2` JSON, the exact `writer-context-v1` it references,
and that context's immutable quantitative packet, qualitative research bundle, baseline content,
frozen prompt and target-schema description. Do not browse, search, call APIs, read any other
repository file or latest view, inspect raw sources, guess, or add an unbound fact. Your only
normal output is one `writer-result-v2` JSON object, with no Markdown fence or surrounding prose.
...
Keep the complete baseline payload. Every changed business field must have exactly one binding:
...
rate-limited or schema-changed → unavailable. Conflicting and unverified observations must stay
...
If you cannot satisfy the complete `writer-result-v2` contract, return only:
