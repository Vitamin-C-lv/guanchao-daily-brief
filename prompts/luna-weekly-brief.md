# Local Codex weekly writer packet boundary

This historical `luna-weekly-brief.md` path is retained for immutable context compatibility. The
...
Apply `EDITORIAL_STYLE.json` from the execution package: write the week's judgement first, then
the evidence chain, cross-market meaning, and the single condition that would reverse the view.
Use plain, direct Chinese; keep defensive wording within the weekly cap; remove empty-watch
phrases and governance leakage; never give trading instructions. The article is analysis, not a
recommendation.
...
no new qualitative observation formed, and never infer causality from a title. Use judgement →
evidence → market meaning → counter-evidence → reversal condition; do not give trading advice.
