...
- prediction automation dry-run 状态：在临时 runtime（`D:\Guanchao-Workspace\temp\stage3-dry-run-runtime`，分支 `feature/stage3-prediction-product-release`，origin=GitHub）执行完整 `run-prediction-publisher.mjs --edition-date 2026-08-06 --dry-run`；runtime pull 与 market-data refresh 通过；A 股 rotation refresh 被 CSI `403 Forbidden` 阻断（WAF/rate gate，项目政策禁止绕过与供应商回退，按两次尝试上限停止），publisher 因此报告 `INFER_FAILED`，未进入 stage3 新步骤。阶段三新增步骤已用同一确定性脚本单独验证：publication gate（15 horizon 全 blocked）、DTO build+validate、ledger states sandbox append/verify 幂等全部通过；全程未写生产、未 commit、未 push。
...
- 环境困难（environmentDifficulties）：完整 prediction dry-run 首次执行时 A 股 rotation refresh 报 `datetime.time has no attribute 'sleep'`——冻结基线 `scripts/sector_rotation.py` 第 35 行 `from datetime import ... time` 遮蔽了 `time` 模块，导致任何重试/限流路径的 `time.sleep` 崩溃（CSI 403/WAF 时会触发）。最小修复：`from datetime import time as dt_time` 并改用 `dt_time(17, 0)`（`datetime.time(17,0)` 会取 datetime 类描述符），行为不变；不涉及模型、门槛或数据语义。修复后暴露的真实阻断为 CSI 403（provider unavailable，fail-closed，不绕过 WAF）。
- 最终集中修复（2026-08-06，同 PR #52）：① 修复 HK/US `sourceStatus` 跨市场污染——每个市场只汇总自身 registry `requiredSourceRoles` 的真实来源状态，US 不再继承 HSTECH 失败，DTO 不公开 raw source ID，reason 全中文；② 公开状态原因/目标/数据状态/candidate 文案全中文化（DTO claim/statusReason/sourceStatus.reason 为自然中文，页面经单一标签映射显示中文，不显示原始枚举与英文内部句子）；③ datasetId/modelVersion 降为“技术信息”折叠区，主摘要只显示数据截至、数据状态中文与是否发布概率；④ 截图流程仅关闭周报弹窗（不改生产弹窗行为），11 张截图重新生成且 `weeklyModalCount=0`、`modalBackdropCount=0`、`visibleEnglishInternalTokenCount=0`；⑤ 完整测试通过（gate 10/10、DTO 6/6、publisher 10/10、ledger 39/39+22/22、DOM 6/6、`pnpm check` 含 build 通过），并覆盖原 Review ZIP。CSI 403 继续如实记录为 provider blocked，不绕过、不伪造 live dry-run 成功。
