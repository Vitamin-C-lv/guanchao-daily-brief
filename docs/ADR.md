...
| ADR-016 | 研究资料必须经不可变 source run、document 和 research bundle 契约后才可进入 writer context。 | Luna 不得直接浏览或读取任意 latest 文件；未来 `writer-context-v1` 将扩展 ADR-012，同时保留“Luna 不得自主浏览”的核心原则。 |
| ADR-017 | 媒体网页只保留元数据、哈希和最小证据片段。 | 不保存完整新闻正文；不得绕过付费墙或任何访问控制，避免版权与合规风险。 |
| ADR-018 | evidenceState 由来源类别、独立 publisher 数量和反证关系确定性派生。 | AI 不得自行提升证据等级；社区和社交线索保留为线索或主体声明，而非确认事实。 |
