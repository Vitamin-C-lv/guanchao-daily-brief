...
research bundle 不可直接交给 Luna。`writer-context-v1` 同时引用 immutable quantitative writer packet（schema version、artifact path/SHA、writerPacketId）、qualitative research bundle（schema version、artifact path/SHA、bundleId）和 baseline content（schema version、artifact path/SHA、contentIdentity），并绑定 `writerPromptSha256`、`targetSchemaVersion`、`validatorSha256` 及 prompt、target schema 与 validator 的路径和 SHA。`writer-request-v2` 只绑定这个显式 context，并扩展 ADR-012 与 P1-F 的 writer-request 边界；Luna 仅接收封闭 execution package，不得浏览、搜索、调用 API 或读取 latest。本阶段绝不修改 P1-F 代码。
