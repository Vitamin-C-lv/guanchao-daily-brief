...
→ immutable run → writer packet → Luna → content validation → prediction ledger.
...
4. Supply only `content/writer-packets/daily-latest.json` and the existing content schema to Luna.
5. Validate generated content, then run `pnpm check`. Only after those checks may the
...
Use the same order with `--edition weekly`, then give
`content/writer-packets/weekly-latest.json` to Luna. Validate the brief, run `pnpm check`,
